...
int main()
{
    ...
    {
        std::cout << "Error al leer del socket." << std::endl;
        return -1;
    }
    ...
    system("clear");
    valread = read(sock, buffer, 1024); // read --> Lee datos desde el socket conectado al servidor y los almacena en el buffer
    if (valread > 0)
    {
        buffer[valread] = '\0'; // Null-terminate the buffer
        // std::cout << buffer << std::endl;
    ...
    std::thread enviar_thread([sock, username]
                              { enviarMensajes(sock, username); }); // --> Crear un hilo para enviar mensajes al servidor usando una lambda function
